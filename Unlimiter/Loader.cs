...
namespace TreeUnlimiter
{
    public class Loader : LoadingExtensionBase
    {
        ...
        public override void OnCreated(ILoading loading)
        {
            ...
            if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true) { Debug.Log("[TreeUnlimiter:Loader::OnCreated]"); }
            ...
            {
                if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true) { Debug.Log("[TreeUnlimiter:Loader::OnCreated]  enabled and redirects not setup yet"); }
...
*/
        ...
        {
            if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true) { Debug.Log("[TreeUnlimiter:Loader::OnLevelLoaded]  LoadMode:" + mode.ToString()); }
            if (Mod.IsEnabled == true & Mod.IsSetupActive == false)
            {
                ...
                {
                    if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true) { Debug.Log("[TreeUnlimiter:Loader::OnLevelLoaded]  AssetModeNotDetcted"); }
            ...
            {
                if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true) { Debug.Log("[TreeUnlimiter:Loader::OnLevelLoaded] enabled and setup already"); }
                if (mode == LoadMode.LoadAsset || mode == LoadMode.NewAsset)
                {
                    //if we are asseteditor then revert the redirects, and reset the treemanager data.
                    if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true) { Debug.Log("[TreeUnlimiter:Loader::OnLevelLoaded]  AssetModeDetcted, removing redirects and resetting treemanager"); }
                    ...
                    {
                        if (Mod.DEBUG_LOG_ON) { Debug.Log("[TreeUnlimiter:Loader:OnLevelLoaded]  AssetModeDetcted, Treecount is less < then 0 !"); };
...
//                    uint inum;
                    ...
                    {
                        if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true) { Debug.Log("[TreeUnlimiter:Loader::OnLevelLoaded]  New or LoadMap Detected - itemcount==0 treecount == -1"); }
...
//                        }
            ...
            if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true)  //Debugging crap for the above stated hack.
            {
                ...
                Debug.Log("[TreeUnlimiter:Loader::OnLevelLoaded] Debugging-TreeManager: treecount=" + mtreecountr + " msize=" + mtreebuffsize + " mbuffleg=" + mtreebuffleg + " buffitemcount=" + mtreebuffcount + " UpdatedTreesSize=" + mupdtreenum);
        ...
        public override void OnLevelUnloading()
        {
            if (Mod.IsEnabled == true | Mod.IsSetupActive == true)
            {
                if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true) { Debug.Log("[TreeUnlimiter:Loader::OnLevelUnloading]"); }
        ...
        {
            if (TreeUnlimiter.Mod.DEBUG_LOG_ON == true) { Debug.Log("[TreeUnlimiter:Loader::OnReleased]"); }
            if (Mod.IsEnabled == true | Mod.IsSetupActive == true)
            {
